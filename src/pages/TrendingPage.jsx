...
const TrendingPage = () => {
  ...
  useEffect(() => {
    const fetchTrendingCountries = async () => {
      ...
      try {
        ...
        setError(
          'Failed to load trending countries. Please try again later.'
        );
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen">
        <UserHeader />
        <main className="flex-grow container mx-auto px-4 py-8 animate-pulse space-y-4">
          {[...Array(5)].map((_, i) => (
  ...
  if (error) {
    return (
      <div className="flex flex-col min-h-screen">
        <UserHeader />
        <main className="flex-grow max-w-4xl mx-auto py-12 px-4 text-center">
